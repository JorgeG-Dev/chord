use crate::{git, manifest, utils, workspace};
use serde_saphyr;
use std::{fs::File, path::PathBuf};

use anyhow::{Result, bail};

pub fn run(backend: &impl git::Backend) -> Result<()> {
    ...
    let manifest: manifest::Manifest = serde_saphyr::from_reader(manifest_file)?;
    let workspace = workspace::Workspace::try_from(manifest)?;
